...
{   // #### mod 7e: added switch for autoISF ON/OFF
 ...
 function getSensValue(profile, autosens_data) {
    const preferences = storage.retrieve(OpenAPS.Settings.preferences); // assuming storage is accessible
    if (!preferences) {
        console.error("Error loading preferences in getSensValue");
        return profile.sens; // Return default sensitivity if preferences cannot be loaded
    }

    const useAutosensIsfToCalculateAutoIsfSens = preferences.useAutosensIsfToCalculateAutoIsfSens;
    let sensValue = profile.sens; // Default to profile sensitivity

    if (useAutosensIsfToCalculateAutoIsfSens && autosens_data && autosens_data.ratio) {
        sensValue = profile.sens / autosens_data.ratio;
    }

    return sensValue;
} //end of 14k code block
    ...
    if (bg_ISF<1) {
        ...
        isfreason += ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile, autosens_data),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        return autoISFsens;
    ...
    if ( sens_modified ) {
        ...
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile, autosens_data),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        return autoISFsens;
    }
...
// Calculate sensitivityRatio based on temp targets, if applicable, or using the value calculated by autosens
    ...
    else if (typeof autosens_data !== 'undefined' && autosens_data && profile.enable_autosens && getSensValue(profile, autosens_data) !== profile.sens) {
