...
function getTddAdjustedMaxIob(profile, glucose_status, target_bg) {
    ...
    if (profile.enable_max_iob_deadbands) {
        ...
        if (currentBG >= 0 && currentBG < tightBandUpperLimit) {
            // Apply tight deadband limits when current BG is between 0 and the tight upper limit
            return profile.max_iob_tight_deadband / 100 * tdd;
        } else if (currentBG >= tightBandUpperLimit && currentBG <= looseBandUpperLimit) {
            // Apply loose deadband limits when current BG is between the tight upper limit and the loose upper limit
...
function getTddAdjustedMaxIobValue(profile, glucose_status, target_bg) {
    ...
    if (profile.enable_max_iob_deadbands) {
        ...
        if (currentBG >= 0 && currentBG < tightBandUpperLimit) {
            // Apply tight deadband limits when current BG is between 0 and tightUpperLimit
            return profile.max_iob_tight_deadband;
        } else if (currentBG >= tightBandUpperLimit && currentBG <= looseBandUpperLimit) {
            // Apply loose deadband limits when current BG is between tightUpperLimit and the loose upper limit
...
// Print summary variables based on predBGs etc.
    ...
    let tddISFaf = "";
    if (profile.enable_max_iob_deadbands){
        maxIOBreason2 = "tddISF-AF: " + round(profile.calculate_isf_from_tdd_numerator_divisor,2);
    }
// Didn't remove anything    rT.reason =  MWreason + TddAdjustBasalreason + maxIOBreason2 + B30reason + SMBdelreason + autosensReason + TTreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);  
    rT.reason =  MWreason + TddAdjustBasalreason + maxIOBreason2 + tddISFaf + B30reason + autosensReason + TTreason + isfreason + ", Standard" + ", BGI: " + rT.BGI + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
