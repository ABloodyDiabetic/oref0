...
// Print summary variables based on predBGs etc.
    ...
    if ( maxCOBPredBG > bg ) {
       ...
       if ( profile.sensitivity_raises_target && 1.5 < 1 || profile.resistance_lowers_target && 1.5 > 1 ) {
         // with a target of 100, default 0.7-1.2 autosens min/max range would allow a 93-117 target range
         min_bg = round((min_bg - 60) / 1.5) + 60;
         max_bg = round((max_bg - 60) / 1.5) + 60;
         var new_target_bg = round((target_bg - 60) / 1.5) + 60;
         // don't allow target_bg below 80
          new_target_bg = Math.max(80, new_target_bg);
          if (target_bg === new_target_bg) {
              process.stderr.write("target_bg unchanged: "+new_target_bg+"; ");
