...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables, middleWare) {
   ...
   var tdd_before = get24HrTDD (profile);
   ...
   // -------------------- END OF TDD ----------------------------------------------------
    ...
    function get24HrTDD (profile) {
      if (tdd == 0) {
        return profile.twentyFourHrTDDPlaceholder
      }
      return tdd
    }

      function get7DayTDD (profile) {
      if (tdd == 0) {
        return profile.sevenDayTDDPlaceholder
      }
      return average_7days
    }

/*      function getSensValue(profile, value) {
        const tddAdjustedISF = 1700 / profile.calculate_isf_from_tdd_numerator_divisor / get24HrTDD (profile);
          
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens) {
            return tddAdjustedISF;
        }
        return profile.sens;
    } // This function is the toggle to enable or disable "Use Autosens ISF to Calculate the autoISF Sens". 'getSensValue(profile)' has replaced 'profile.sens' in several places.*/
    ...
    function getSensValue(profile, value) {
        const tddAdjustedISF = (profile.isfSlope * get24HrTDD (profile)) + profile.isfIntercept;
...
/*    function getBasalValue(profile, value) {
        const originalBasal = profile.current_basal;
        const originalISF = profile.sens;
        const tddAdjustedISF = 1700 / profile.calculate_isf_from_tdd_numerator_divisor / get24HrTDD (profile);

        if (profile.use_autosens_isf_to_calculate_auto_isf_sens) {
            const isfRatio = originalISF / tddAdjustedISF;
            let newBasal = originalBasal * isfRatio;
            newBasal = Math.max(0, Math.round((newBasal * profile.basal_multiplier)/ 0.05) * 0.05);
         return newBasal;
    }

    return originalBasal;
} // This function is the toggle to enable or disable "Adjust Basal Inversely to Autosens ISF". 'getBasalValue(profile)' replaces 'profile.current_basal' in several places.*/
  
    function getBasalValue(profile, value) {
        const originalBasal = profile.current_basal;

        if (profile.use_autosens_isf_to_calculate_auto_isf_sens) {
            let newBasal = (profile.basalSlope * get24HrTDD (profile)) + profile.basalIntercept;
            newBasal = Math.max(0.05, Math.round((newBasal * profile.basal_multiplier)/ 0.05) * 0.05);
         return newBasal;
    }
...
function getTddAdjustedMaxIob(profile, glucose_status, target_bg) {
    const currentBG = glucose_status.glucose;
  
    if (profile.enable_max_iob_deadbands) {
        const tightBandUpperLimit = (profile.tight_deadband_range / 100 * target_bg) + target_bg;
        const looseBandUpperLimit = (profile.loose_deadband_range / 100 * target_bg) + target_bg;

        if (currentBG >= 0 && currentBG < tightBandUpperLimit && !profile.moderate_carb_ez_fcl_profile && !profile.high_carb_ez_fcl_profile) {
            // Apply tight deadband limits when current BG is between 0 and the tightBandUpperLimit
            return profile.max_iob_tight_deadband / 100 * get24HrTDD (profile);
        } else if (currentBG >= tightBandUpperLimit && currentBG <= looseBandUpperLimit && !profile.moderate_carb_ez_fcl_profile && !profile.high_carb_ez_fcl_profile) {
            // Apply loose deadband limits when current BG is between the tightBandUpperLimit and the looseBandUpperLimit
            return profile.max_iob_loose_deadband / 100 * get24HrTDD (profile);
        } else if (currentBG >= tightBandUpperLimit && profile.sleep_mode) {
            // Apply tight deadband value whever sleep mode is active
            return profile.max_iob_tight_deadband / 100 * get24HrTDD (profile);
        } else if (currentBG >= tightBandUpperLimit && profile.automatic_sleep_mode) {
            // Apply tight deadband value whever automatic sleep mode is active and Apple Health sleep data indicates a sleep state
            return profile.max_iob_tight_deadband / 100 * get24HrTDD (profile);             
        }
    }
    
    // Default return if no conditions are met
    return profile.max_iob / 100 * get7DayTDD (profile);
} // This function modifies maxIOB to equal a percentage of the TDD to allow dynamic adjustment relative to TDD instead of being a fixed number. Everywhere you see getTddAdjustedMaxIob(profile, glucose_status, target_bg) is a location where 'profile.max_iob' was replaced.

function getTddAdjustedMaxIobValue(profile, glucose_status, target_bg) {
    const currentBG = glucose_status.glucose;
    ...
    if (profile.enable_max_iob_deadbands) {
        ...
        if (currentBG >= 0 && currentBG < tightBandUpperLimit && !profile.moderate_carb_ez_fcl_profile && !profile.high_carb_ez_fcl_profile) {
            // Apply tight deadband limits when current BG is between 0 and tightBandUpperLimit
            return profile.max_iob_tight_deadband;
        } else if (currentBG >= tightBandUpperLimit && currentBG <= looseBandUpperLimit && !profile.moderate_carb_ez_fcl_profile && !profile.high_carb_ez_fcl_profile) {
            // Apply loose deadband limits when current BG is between tightBandUpperLimit and looseBandUpperLimit
            return profile.max_iob_loose_deadband;
        } else if (currentBG >= 0 && profile.sleep_mode) {
            // Apply tight deadband value whever sleep mode is active
            return profile.max_iob_tight_deadband;
        } else if (currentBG >= 0 && profile.automatic_sleep_mode) {
            // Apply tight deadband value whever automatic sleep mode is active and Apple Health sleep data indicates a sleep state
            return profile.max_iob_tight_deadband;          
        }
    }
    ...
    // Default return if no conditions are met
    return profile.max_iob;
} // This function returns the maxIOB value used to determine the maxIOB value calculated against the TDD. This is only used for maxIOBreason.
...
function enable_smb(
    ...
    value)
    {
    // disable MB when a high temptarget is set
    if (! microBolusAllowed) {
        console.error("MB disabled (!microBolusAllowed)");
        return false;
    } else if (! profile.allowSMB_with_high_temptarget && profile.temptargetSet && target_bg > 100) {
        console.error("MB disabled due to high temptarget of " + target_bg);
        return false;
    } else if (meal_data.bwFound === true && profile.A52_risk_enable === false) {
        console.error("MB disabled due to Bolus Wizard activity in the last 6 hours.");
        return false;
    }

    // enable MB/UAM if always-on (unless previously disabled for high temptarget)
    if (profile.sleep_mode === true && profile.disable_mb_during_sleep || profile.automatic_sleep_mode === true && profile.disable_mb_during_sleep) {
        console.error("MB disabled due to sleep mode.");
        return false; // Disable MBs when sleep mode is active
    }

    if (profile.enableSMB_always === true) {
        if (meal_data.bwFound) {
            console.error("Warning: MB enabled within 6h of using Bolus Wizard: be sure to easy bolus 30s before using Bolus Wizard");
        } else {
            console.error("MB enabled due to enable MBs");
        }
        return true;
    }

    // enable MB/UAM (if enabled in preferences) while we have COB
    if (profile.enableSMB_with_COB === true && meal_data.mealCOB) {
        if (meal_data.bwCarbs) {
            console.error("Warning: MB enabled with Bolus Wizard carbs: be sure to easy bolus 30s before using Bolus Wizard");
        } else {
            console.error("MB enabled for COB of" + meal_data.mealCOB);
        }
        return true;
    }

    // enable MB/UAM (if enabled in preferences) for a full 6 hours after any carb entry
    // (6 hours is defined in carbWindow in lib/meal/total.js)
    if (profile.enableSMB_after_carbs === true && meal_data.carbs ) {
        if (meal_data.bwCarbs) {
            console.error("Warning: MB enabled with Bolus Wizard carbs: be sure to easy bolus 30s before using Bolus Wizard");
        } else {
            console.error("MB enabled for 6h after carb entry");
        }
        return true;
    }

    // enable MB/UAM (if enabled in preferences) if a low temptarget is set
    if (profile.enableSMB_with_temptarget === true && (profile.temptargetSet && target_bg < 100)) {
        if (meal_data.bwFound) {
            console.error("Warning: MB enabled within 6h of using Bolus Wizard: be sure to easy bolus 30s before using Bolus Wizard");
        } else {
            console.error("MB enabled for temptarget of " + convert_bg(target_bg, profile));
        }
        console.error("MB enabled for temptargets with " + convert_bg(target_bg, profile));
        return true;
    }

    // enable MB if high bg is found
    if (profile.enableSMB_high_bg === true && high_bg !== null && bg >= high_bg) {
        console.error("Checking BG to see if High for MB enablement.");
        console.error("Current BG", bg, " | High BG ", high_bg);
        if (meal_data.bwFound) {
            console.error("Warning: High BG MB enabled within 6h of using Bolus Wizard: be sure to easy bolus 30s before using Bolus Wizard");
        } else {
            console.error("High BG detected. Enabling MB.");
        }
        return true;
    }

    console.error("MB disabled (no enableMB preferences active or no condition satisfied)");
    return false;
}
...
function loop_smb(microBolusAllowed, profile, iob_data, aimismb, iobTH_reduction_ratio) {
    ...
    //     smbreason = ", autoISF-MB disabled:, IOB: " + round(iob_data.iob,2) + ", > threshold: " + profile.iob_threshold + ", maxIOB: " + getTddAdjustedMaxIob(profile, glucose_status, target_bg);
    //     console.error(smbreason);
    //     return "iobTH"
    // }  //not needed as IOB Threshold Percent is used now

    // disable MB when a B30 basal is running
    if (!aimismb) {
        smbreason = ", ezFCL-MB disabled:, B30 running";
        return "AIMI B30";
    }
    if ( !microBolusAllowed ) {
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || (profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always && !profile.temptargetSet))  {
        ...
        if ( !evenTarget ){
            console.error("MB disabled - " + msgType + target + msgUnits + msgEven + msgTail);
            smbreason = ", ezFCL-MB disabled:, odd Target";
            console.error("Loop at minimum power");
            return "blocked";
        } else if ( getTddAdjustedMaxIob(profile, glucose_status, target_bg)==0 ) {
            console.error("MB disabled because of maxIOB=0")
            return "blocked";
        } else if (iobTHeffective/100 < iob_data.iob/(getTddAdjustedMaxIob(profile, glucose_status, target_bg)*iobTH_reduction_ratio)) {
            console.error("iobTH: " + round(iobTHeffective,1) + "%, IOB% of maxIOB at " + round(iob_data.iob/(getTddAdjustedMaxIob(profile, glucose_status, target_bg)*iobTH_reduction_ratio)*100,1) + "%")
            if (iobTH_reduction_ratio != 1) {
                console.error("Full Loop modified maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg) + " to effectively " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg)*iobTH_reduction_ratio,2) + " due to profile % and/or exercise mode");
                maxIOBreason = ", effective maxIOB " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg)*iobTH_reduction_ratio,2);
            } else {
                maxIOBreason = ", maxIOB "+ getTddAdjustedMaxIob(profile, glucose_status, target_bg);
            }
            smbreason = ", ezFCL-MB disabled:, iobTH exceeded";
            console.error("MB disabled by Full Loop logic: IOB " + iob_data.iob + " is more than " + iobTHeffective + "% of maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg));
            console.error("Full Loop capped");
            return "iobTH";
        } else {
            console.error("MB enabled - " + msgType + target + msgUnits + msgEven + msgTail);
            ...
            if (target < 100) {     // indirect asessment; later set it in GUI
                ...
                smbreason = ", ezFCL-MB enabled:, even TT";
                return "fullLoop";                                      // even number
            } else {
                console.error("iobTH: " + round(iobTHeffective,1) + "%, IOB% of maxIOB at " + round(iob_data.iob/(getTddAdjustedMaxIob(profile, glucose_status, target_bg)*iobTH_reduction_ratio)*100,1) + "%")
                smbreason = ", ezFCL-MB enabled:, even Target";
                console.error("Loop at medium power");
                return "enforced";
            }
        }
    }
    //console.error("-- end checking advanced MB logic ---------")
...
function withinISFlimits(liftISF, minISFReduction, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget) {
    ...
    if ( high_temptarget_raises_sensitivity && profile.temptargetSet && target_bg > normalTarget ) {
         ...
         console.error("autoISF adjusts sens " + sensitivityRatio + ", instead of getSensValue(profile) "  + getSensValue(profile));
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if  (!profile.enable_BG_acceleration) {
        ...
        if (parabola_fit_a2 !=0 && fit_corr>=0.9) {
            ...
            if (minmax_delta>0 && bg_acce<0) {
                fitreason = "Max of " + convert_bg(minmax_value,profile) + " in " + Math.abs(minmax_delta) + " min";
                console.error("Parabolic fit " + fitreason);
            } else if (minmax_delta>0 && bg_acce>0) {
                fitreason = "Min of " + convert_bg(minmax_value,profile) + " in " + Math.abs(minmax_delta) + " min";
                console.error("Parabolic fit " + fitreason);
                if (minmax_delta<=30 && minmax_value<target_bg) {   // start braking
                    acce_weight = -profile.bgBrake_ISF_weight;
                    fitreason = "Low BG soon. bgBrake ISF weight: " + -acce_weight;
                    console.error("Parabolic fit " + fitreason);
                }
            } else if (minmax_delta<0 && bg_acce<0) {
                fitreason = "Max of " + convert_bg(minmax_value,profile) + " " + Math.abs(minmax_delta) + " min ago";
                console.error("Parabolic fit " + fitreason);
            } else if (minmax_delta<0 && bg_acce>0) {
                fitreason = "Min of " + convert_bg(minmax_value,profile) + " " + Math.abs(minmax_delta) + " min ago";
                console.error("Parabolic fit " + fitreason);
            }
        }
        if ( fit_corr<0.9 ) {
            fitreason = ", acce-ISF Ratio: " + round(acce_ISF,2) + " (" + /*"acce-ISF: 1.0 as " +*/ round(fit_corr,2) + " correlation is too low" + ")";
            console.error("Parabolic fit " + fitreason);
            ...
            // calcreason += ", Parabolic Fit, " + fitreason;
            console.error(calcreason + "acce-ISF adaptation is " + round(acce_ISF,2));
            if ( acce_ISF != 1 ) {
                sens_modified = true;
                calcreason += ", acce-ISF Ratio: " + round(acce_ISF,2) + " (" + fitreason + ")";
            }
        }
    ...
    let profileModifiedLiftISF = 1;

    const baseLiftISF = liftISF;

    if (profile.low_carb_ez_fcl_profile) {
        profileModifiedLiftISF = ((baseLiftISF - 1) * profile.low_carb_ez_fcl_profile_multiplier) + 1;
    } else if (profile.moderate_carb_ez_fcl_profile) {
        profileModifiedLiftISF = ((baseLiftISF - 1) * profile.moderate_carb_ez_fcl_profile_multiplier) + 1;
    } else if (profile.high_carb_ez_fcl_profile) {
        profileModifiedLiftISF = ((baseLiftISF - 1) * profile.high_carb_ez_fcl_profile_multiplier) + 1;
    } else {
        // If none of the specific profiles are true, use the base ISF without any modifications.
        profileModifiedLiftISF = baseLiftISF;
    }         
        final_ISF = withinISFlimits(profileModifiedLiftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget);
        autoISFsens = Math.min(720, round(getSensValue(profile) / final_ISF, 1));
    function twoOrThreeStepfinalISFReason(profile, value) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens) {
            return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        }
        return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(autoISFsens, profile);
    } // This function uses the "Use Autosens ISF to Calculate the autoISF Sens" toggle value to determine if the Autosens ISF should be included in the 'final ISF' progression. Two step or three step relative to the toggle.
        //isfreason +=  ", bg-ISF Ratio: " + round(final_ISF,2);
        isfreason += ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + twoOrThreeStepfinalISFReason(profile);
        return autoISFsens;
    ...
    if (bg_off > 0) {
        ...
        ppisfreason = ", Δ-ISF Ratio: " + round(pp_ISF,2);
        if (pp_ISF != 1) {
            sens_modified = true;
        }
    } else {
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        sens_modified = true;
        duraisfreason = ", --- ISF Ratio: " + round(dura_ISF,2) + " (Duration: " + dura05 + " - Avg BG: " + convert_bg(avg05,profile) + "),";
    ...
    if ( sens_modified ) {
        ...
        console.error("  --- " + round(dura_ISF,2));
        console.error("  Δ " + round(pp_ISF,2));
    ...
    if (profile.low_carb_ez_fcl_profile) {
        ...
        final_ISF = withinISFlimits(profileModifiedLiftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget);
        autoISFsens = round(getSensValue(profile) / final_ISF, 1);
    function twoOrThreeStepfinalISFReason2(profile, value) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens) {
            return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        }
        return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(autoISFsens, profile);
    } // This function uses the "Use Autosens ISF to Calculate the autoISF Sens" toggle value to determine if the Autosens ISF should be included in the 'final ISF' progression. Two step or three step relative to the toggle.      
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + twoOrThreeStepfinalISFReason2(profile);
        return autoISFsens;
    }
...
{   // mod 12: let MB delivery ratio increase f#rom min to max depending on how much bg exceeds target
// if ( !profile.use_autoisf ) {
//    console.error("ezFCL disabled, don't adjust MB Delivery Ratio");
//    return 0.5}
    ...
    if ( loop_wanted_smb=='fullLoop' ) {                                // go for max impact
        console.error('MB delivery ratio set to ' + round(Math.max(fix_SMB, new_SMB),2) + ' as max of fixed and interpolated values');
        return Math.max(fix_SMB, new_SMB);
    }
    if ( smb_delivery_ratio_bg_range==0 ) {                     // deactivated in MB extended menu
        console.error('MB delivery ratio set to fixed value ' + round(fix_SMB,2));
        return fix_SMB;
    }
    if (bg <= target_bg) {
        console.error('MB delivery ratio limited by minimum value ' + round(lower_SMB,2));
        return lower_SMB;
    }
    if (bg >= higher_bg) {
        console.error('MB delivery ratio limited by maximum value ' + round(higher_SMB,2));
        return higher_SMB;
    }
    console.error('MB delivery ratio set to interpolated value ' + round(new_SMB,2));
    return new_SMB;
}
//end autoISF
...
// Set variables required for evaluating error conditions
    ...
    if (typeof profile === 'undefined' || typeof getBasalValue(profile) === 'undefined') {
        rT.error ='Error: could not get current basal rate';
        return rT;
    }
    var profile_current_basal = round_basal(getBasalValue(profile), profile);
...
// Cancel high temps (and replace with neutral) or shorten long zero temps for various error conditions
    ...
    var max_iob = getTddAdjustedMaxIob(profile, glucose_status, target_bg); // maximum amount of non-bolus IOB OpenAPS will ever deliver
...
// Calculate sensitivityRatio based on temp targets, if applicable, or using the value calculated by autosens
    ...
    else if (typeof autosens_data !== 'undefined' && autosens_data && profile.enable_autosens && !profile.use_autosens_isf_to_calculate_auto_isf_sens) {
    ...
    if (sensitivityRatio) {
        basal = getBasalValue(profile) * sensitivityRatio;
...
// Conversely, adjust BG target based on autosens ratio if no temp target is running
    ...
    if (profile.temptargetSet) {
        ...
        if ( profile.sensitivity_raises_target && autosens_data.ratio < 1 || profile.resistance_lowers_target && autosens_data.ratio > 1 ) {
            ...
            // don't allow target_bg below 35
            new_target_bg = Math.max(35, new_target_bg);
    ...
    //console.log("MB Threshold set to " + threshold_ratio + " - no MB's applied below " + convert_bg(threshold, profile));
    console.log(/*"MB Threshold set to " + threshold_ratio + */"No MB's applied below " + convert_bg(threshold, profile));
...
// Adjust ISF based on sensitivityRatio
    var profile_sens = round(getSensValue(profile),1);
    var sens = getSensValue(profile);
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        sens = getSensValue(profile) / sensitivityRatio;
        sens = round(sens, 1);
    ...
    if (microBolusAllowed && loop_wanted_smb != "oref") {
        // if ( loop_wanted_smb == "blocked" || loop_wanted_smb == "AIMI B30") {              //  FL switched MB off
        //     enableSMB = false;
        // }
        if ( loop_wanted_smb=="enforced" || loop_wanted_smb=="fullLoop" ) {              // otherwise FL switched MB off
            enableSMB = true;
        }
        console.error("loop_smb function overriden with autoISF checks, enableMB = " + enableSMB);
       ...
       console.error("loop_smb function returns enableMB = " + enableSMB);
    }
...
// This has to happen after we obtain iob_data
    ...
    } else { // if IOB is negative, be more conservative and use the lower of sens, getSensValue(profile)
        naive_eventualBG = round( bg - (iob_data.iob * Math.min(sens, getSensValue(profile)) ) );
    }
...
// Initialize and calculate variables used for predicting BGs
    ...
    console.error("profile.sens:" + convert_bg(getSensValue(profile),profile) +", sens:" + convert_bg(sens,profile) + ", CSF:" + round(csf,1));
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            // add 30m to allow for insulin delivery (MBs or temps)
            insulinPeakTime = 90;
...
// Print summary variables based on predBGs etc.
    ...
    rT.auto_ISFratio = round(getSensValue(profile) / sens, 2);
    ...
    } //  && profile.enableMiddleware

    let TddAdjustBasalreason = "";
    if (profile.use_autosens_isf_to_calculate_auto_isf_sens){
        TddAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile),2) + /* " (tddISF: " + convert_bg(1700 / profile.calculate_isf_from_tdd_numerator_divisor / (get24HrTDD (profile)),profile) +  "), " + */ ", " + "Autosens: " + round(autosens_data.ratio,2) + " (ISF: " + round(profile.sens / autosens_data.ratio,0) + ")" + ", ";
    }

    let maxIOBreason2 = "";
    if (profile.enable_max_iob_deadbands){
        maxIOBreason2 = "maxIOB: " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg),2) + " (" + getTddAdjustedMaxIobValue(profile, glucose_status, target_bg) + "% of TDD)";
    }

    let tddISFaf = "";
    if (profile.enable_max_iob_deadbands){
        tddISFaf = "tddISF-AF: " + round(profile.calculate_isf_from_tdd_numerator_divisor,2) + ", ";
    }
// Didn't remove anything    rT.reason =  MWreason + TddAdjustBasalreason + maxIOBreason2 + B30reason + SMBdelreason + autosensReason + TTreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);  
    rT.reason =  MWreason + /* tddISFaf + */ TddAdjustBasalreason + maxIOBreason2 + B30reason + autosensReason + TTreason + isfreason + ", Standard" + ", BGI: " + rT.BGI + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
...
// Use minGuardBG to prevent overdosing in hypo-risk situations
    ...
    if (enableSMB && minGuardBG < threshold) {
        console.error("minGuardBG " + convert_bg(minGuardBG, profile) + " projected below " + convert_bg(threshold, profile) + " - disabling MB");
        //rT.reason += "minGuardBG "+minGuardBG+"<"+threshold+": MB disabled; ";
    ...
    // maxDelta_bg_threshold has a cap at 0.4 as a safety limit, default is 0.2
    ...
    if ( maxDelta > maxDelta_bg_threshold * bg ) {
        console.error("maxDelta " + convert_bg(maxDelta, profile)+ " > " + 100 * maxDelta_bg_threshold + "% of BG " + convert_bg(bg, profile) + " - disabling MB");
        rT.reason += "maxDelta " + convert_bg(maxDelta, profile) + " > " + 100 * maxDelta_bg_threshold + "% of BG " + convert_bg(bg, profile) + " - MB disabled!, ";
        enableSMB = false;
    }
...
// Calculate carbsReq (carbs required to avoid a hypo)
    ...
    var zeroTempEffect = getBasalValue(profile)*sens*zeroTempDuration/60;
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    var durationReq = 0;
    if (bg < threshold && iob_data.iob < -getBasalValue(profile)*20/60 && minDelta > 0 && minDelta > expectedDelta) {
        rT.reason += "IOB "+iob_data.iob+" < " + round(-getBasalValue(profile)*20/60,2);
    ...
    // predictive low glucose suspend mode: BG is / is projected to be < threshold
        ...
        durationReq = round(60*worstCaseInsulinReq / getBasalValue(profile));
        durationReq = round(durationReq/30)*30;
        // always set a 30-120m zero temp (oref0-pump-loop will let any longer MB zero temp run)
    ...
    if ( profile.skip_neutral_temps && rT.deliverAt.getMinutes() >= 55 ) {
        if (!enableSMB) {
            rT.reason += "; Canceling temp at " + (60 - rT.deliverAt.getMinutes()) + "min before turn of the hour to avoid beeping of MDT. MB disabled anyways.";
            return tempBasalFunctions.setTempBasal(0, 0, profile, rT, currenttemp);
        } else {
             console.error((60 - rT.deliverAt.getMinutes()) + "min before turn of the hour, but MB's are enabled - no skipping neutral temps")
        }
    }
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        if ( minDelta > expectedDelta && minDelta > 0 && !carbsReq ) {
            // if naive_eventualBG < 40, set a 30m zero temp (oref0-pump-loop will let any longer MB zero temp run)
    ...
    if (minDelta < expectedDelta) {
        ...
        // if in MB mode, don't cancel MB zero temp
    ...
    if (iob_data.iob > getTddAdjustedMaxIob(profile, glucose_status, target_bg)) {
        rT.reason += "IOB " + round(iob_data.iob,2) + " > maxIOB " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg),2);
        if (currenttemp.duration > 15 && (round_basal(basal, profile) === round_basal(currenttemp.rate, profile))) {
            rT.reason += ", temp " + currenttemp.rate + " ~ req " + basal + "U/hr. ";
            return rT;
        } else {
        ...
        insulinForManualBolus = round((eventualBG - target_bg) / sens, 2);
        // if that would put us over getTddAdjustedMaxIob(profile, glucose_status, target_bg), then reduce accordingly
        if (insulinReq > getTddAdjustedMaxIob(profile, glucose_status, target_bg) - iob_data.iob) {
            rT.reason += "maxIOB " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg), 2) + ", ";
            console.error("InsReq " + round(insulinReq,2) + " capped at " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg) - iob_data.iob,2) + " to not exceed maxIOB");
            insulinReq = getTddAdjustedMaxIob(profile, glucose_status, target_bg) - iob_data.iob;
        } else { console.error("MB not limited by maxIOB (insulinReq: " + insulinReq + " U)");}

        if (insulinForManualBolus > getTddAdjustedMaxIob(profile, glucose_status, target_bg) - iob_data.iob) {
            console.error("Ev. Bolus limited by maxIOB to " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg) - iob_data.iob,2) + " (insulinForManualBolus: " + insulinForManualBolus + " U)");
            // rT.reason += "getTddAdjustedMaxIob(profile, glucose_status, target_bg) " + getTddAdjustedMaxIob(profile, glucose_status, target_bg) + ", ";
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if ( !profile.use_autoisf ) {
              console.error("ezFCL disabled, MB range extension disabled");
              var smb_max_range = 1;
            } else {
              var smb_max_range = profile.smb_max_range_extension;
            }
            if (smb_max_range > 1) {
                console.error("MB max range extended from default by factor "+smb_max_range)
            }
            var maxBolus = 0;
            if (typeof profile.maxSMBBasalMinutes === 'undefined' ) {
                maxBolus = round(smb_max_range * getBasalValue(profile) * 30 / 60 ,1);
                console.error("profile.maxSMBBasalMinutes undefined: defaulting to 30m");
            //if (typeof profile.maxSMBBasalMinutes === 'undefined' ) {
            //    var maxBolus = round( getBasalValue(profile) * 30 / 60 ,1);
            //    console.error("profile.maxSMBBasalMinutes undefined: defaulting to 30m");
            // if IOB covers more than COB, limit maxBolus to 30m of basal
                ...
                if (profile.maxUAMSMBBasalMinutes) {
                    console.error("profile.maxUAMSMBBasalMinutes:",profile.maxUAMSMBBasalMinutes,"getBasalValue(profile):",getBasalValue(profile));
                    maxBolus = round( smb_max_range * getBasalValue(profile) * profile.maxUAMSMBBasalMinutes / 60 ,1);
                } else {
                    console.error("profile.maxUAMSMBBasalMinutes undefined: defaulting to 30m");
                    maxBolus = round( getBasalValue(profile) * 30 / 60 ,1);
                }
            } else {
                console.error("profile.maxSMBBasalMinutes:",profile.maxSMBBasalMinutes,"getBasalValue(profile):",getBasalValue(profile));
                maxBolus = round( smb_max_range * getBasalValue(profile) * profile.maxSMBBasalMinutes / 60 ,1);
            }
            ...
            // if ( !profile.use_autoisf ) {
            //    console.error("autoISF disabled, don't adjust MB Delivery Ratio");
            //    smb_ratio = 0.5;
            // } else {
            //     smb_ratio = determine_varSMBratio(profile, bg, target_bg, loop_wanted_smb);
            // }
            if ( smb_ratio > 0.5) {
                console.error("MB Delivery Ratio increased from default 0.5 to " + round(smb_ratio,2))
            }
            ...
            var iobTHvirtual = iob_threshold_percent/100*iobTHtolerance/100 * getTddAdjustedMaxIob(profile, glucose_status, target_bg) * iobTH_reduction_ratio;
            if (microBolus > iobTHvirtual - iob_data.iob && (loop_wanted_smb=="fullLoop" || loop_wanted_smb=="enforced")) {
                ...
                //    console.error("Full loop modified getTddAdjustedMaxIob(profile, glucose_status, target_bg)", getTddAdjustedMaxIob(profile, glucose_status, target_bg), "to effectively", round(getTddAdjustedMaxIob(profile, glucose_status, target_bg)*profile.profile_percentage/100,1), "due to profile percentage");
                //}
                iobTHreason = ", capped by autoISF iobTH";
                console.error("ezFCL capped MB at " + round(microBolus,2) + " to not exceed " + iobTHtolerance + "% of effective iobTH " + round(iobTHvirtual/iobTHtolerance*100,2) + "U");
            }
            ...
            // if insulinReq > 0 but not enough for a microBolus, don't set an MB zero temp
            ...
            if (durationReq <= 0) {
                durationReq = 0;
            // don't set an MB zero temp longer than 60 minutes
            } else if (durationReq >= 30) {
                durationReq = round(durationReq/30)*30;
                durationReq = Math.min(60,Math.max(0,durationReq));
            } else {
                // if MB durationReq is less than 30m, set a nonzero low temp
            ...
            //allow MBs every 3 minutes by default
            var SMBInterval = 3;
        ...
        var maxSafeBasal = getBasalValue(profile) / profile.current_basal * tempBasalFunctions.getMaxSafeBasal(profile);
