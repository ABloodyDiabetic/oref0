...
function calculate_expected_delta(target_bg, eventual_bg, bgi, value) {
...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables, middleWare, value) {
   ...
   function addTimeToDate(objDate, _hours, value) {
   ...
   function subtractTimeFromDate(date, hours_, value) {
   ...
   function accountForIncrements(insulin, value) {
   ...
   function makeBaseString(base_timeStamp, value) {
       function addZero(i, value) {
           if (i < 10) { i = "0" + i }
           return i;
       }
   ...
   function timeDifferenceOfString(string1, string2, value) {
   ...
   function calcScheduledBasalInsulin(lastRealTempTime, addedLastTempTime, value) {
       var totalInsulin = 0;
   ...
   // -------------------- END OF TDD ----------------------------------------------------
    ...
    function getSensValue(profile, value) {
        const tddAdjustedISF = 1700 / profile.calculate_isf_from_tdd_numerator_divisor / ((tdd * 1) + (tdd_ytd * 0));
    ...
    function getTddAdjustedMaxIob(profile, value, target_bg) {
        if (profile.enable_max_iob_deadbands && 
        Math.abs(convert_bg(value, profile) - target_bg) <= (profile.tight_deadband_range / 100 * target_bg)) {
        return profile.max_iob_tight_deadband / 100 * ((tdd * 1) + (tdd_ytd * 0));
        } else if (profile.enable_max_iob_deadbands && 
               Math.abs(convert_bg(value, profile) - target_bg) > (profile.tight_deadband_range /100 * target_bg) && 
               Math.abs(convert_bg(value, profile) - target_bg) <= (profile.loose_deadband_range /100 * target_bg)) {
        return profile.max_iob_loose_deadband / 100 * ((tdd * 1) + (tdd_ytd * 0));
        } else {
        return profile.max_iob / 100 * ((tdd * 1) + (tdd_ytd * 0));
    }
} // This function modifies maxIOB to equal a percentage of the TDD to allow dynamic adjustment relative to TDD instead of being a fixed number. Everywhere you see getTddAdjustedMaxIob(profile, value, target_bg) is a location where 'profile.max_iob' was replaced.

function getTddAdjustedMaxIobValue(profile, value, target_bg) {
    // Calculating values outside the conditions to avoid repeated calculations
    const convertedValue = convert_bg(value, profile);
    const difference = Math.abs(convertedValue - target_bg);
    const tightThreshold = profile.tight_deadband_range / 100 * target_bg;
    const looseThreshold = profile.loose_deadband_range / 100 * target_bg;

    // Log outside to capture comparison data before conditions
    console.log("Converted BG:", convertedValue);
    console.log("Difference from Target BG:", difference);
    console.log("Tight Threshold:", tightThreshold);
    console.log("Loose Threshold:", looseThreshold);

    if (profile.enable_max_iob_deadbands && difference <= tightThreshold) {
        console.log("Within Tight Deadband");
        return profile.max_iob_tight_deadband;
    } else if (profile.enable_max_iob_deadbands && 
               difference > tightThreshold && 
               difference <= looseThreshold) {
        console.log("Within Loose Deadband");
        return profile.max_iob_loose_deadband;
    } else {
        console.log("Outside Any Deadband");
...
function loop_smb(microBolusAllowed, profile, iob_data, aimismb, iobTH_reduction_ratio, value) {
    ...
    //     smbreason = ", autoISF-SMB disabled:, IOB: " + round(iob_data.iob,2) + ", > threshold: " + profile.iob_threshold + ", maxIOB: " + getTddAdjustedMaxIob(profile, value, target_bg);
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || (profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always && !profile.temptargetSet))  {
        ...
        if ( !evenTarget ){
            ...
            return "blocked";
        } else if ( getTddAdjustedMaxIob(profile, value, target_bg)==0 ) {
            console.error("SMB disabled because of maxIOB=0")
            return "blocked";
        } else if (iobTHeffective/100 < iob_data.iob/(getTddAdjustedMaxIob(profile, value, target_bg)*iobTH_reduction_ratio)) {
            console.error("iobTH: " + round(iobTHeffective,1) + "%, IOB% of maxIOB at " + round(iob_data.iob/(getTddAdjustedMaxIob(profile, value, target_bg)*iobTH_reduction_ratio)*100,1) + "%")
            if (iobTH_reduction_ratio != 1) {
                console.error("Full Loop modified maxIOB " + getTddAdjustedMaxIob(profile, value, target_bg) + " to effectively " + round(getTddAdjustedMaxIob(profile, value, target_bg)*iobTH_reduction_ratio,2) + " due to profile % and/or exercise mode");
                maxIOBreason = ", effective maxIOB " + round(getTddAdjustedMaxIob(profile, value, target_bg)*iobTH_reduction_ratio,2);
            } else {
                maxIOBreason = ", maxIOB "+ getTddAdjustedMaxIob(profile, value, target_bg);
            }
            smbreason = ", autoISF-SMB disabled:, iobTH exceeded";
            console.error("SMB disabled by Full Loop logic: IOB " + iob_data.iob + " is more than " + iobTHeffective + "% of maxIOB " + getTddAdjustedMaxIob(profile, value, target_bg));
...
function interpolate(xdata, profile, type, value)
...
function withinISFlimits(liftISF, minISFReduction, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget, value) {
...
function autoISF(sens, origin_sens, target_bg, profile, glucose_status, meal_data, currentTime, autosens_data, sensitivityRatio, loop_wanted_smb, high_temptarget_raises_sensitivity, normalTarget, value)
...
function determine_varSMBratio(profile, bg, target_bg, loop_wanted_smb, value)
...
// Cancel high temps (and replace with neutral) or shorten long zero temps for various error conditions
    ...
    var max_iob = getTddAdjustedMaxIob(profile, value, target_bg); // maximum amount of non-bolus IOB OpenAPS will ever deliver
...
// Initialize and calculate variables used for predicting BGs
    ...
    var predCIs = [];
    try {
        iobArray.forEach(function(iobTick, value) {
    ...
    // }
    rT.predBGs = {};
    IOBpredBGs.forEach(function(p, i, theArray, value) {
    ...
    ZTpredBGs.forEach(function(p, i, theArray, value) {
        theArray[i] = round(Math.min(401,Math.max(39,p)));
    });
    ...
    if (meal_data.mealCOB > 0 && ( ci > 0 || remainingCIpeak > 0 )) {
        COBpredBGs.forEach(function(p, i, theArray, value) {
            theArray[i] = round(Math.min(401,Math.max(39,p)));
        });
    ...
    if (ci > 0 || remainingCIpeak > 0) {
        if (enableUAM) {
            UAMpredBGs.forEach(function(p, i, theArray, value) {
                theArray[i] = round(Math.min(401,Math.max(39,p)));
            });
...
// Print summary variables based on predBGs etc.
    ...
    if (profile.use_autosens_isf_to_calculate_auto_isf_sens){
        TddAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile),2) + " (tddISF: " + convert_bg(1700 / profile.calculate_isf_from_tdd_numerator_divisor / ((tdd * 1) + (tdd_ytd * 0)),profile) + "), ";
    }

    let maxIOBreason2 = "";
    if (profile.enable_max_iob_deadbands){
        maxIOBreason2 = "maxIOB: " + getTddAdjustedMaxIob(profile, value, target_bg) + ", ";
    }
  
    rT.reason =  MWreason + TddAdjustBasalreason + maxIOBreason2 + B30reason + SMBdelreason + autosensReason + TTreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            var iobTHvirtual = iob_threshold_percent/100*iobTHtolerance/100 * getTddAdjustedMaxIob(profile, value, target_bg) * iobTH_reduction_ratio;
            if (microBolus > iobTHvirtual - iob_data.iob && (loop_wanted_smb=="fullLoop" || loop_wanted_smb=="enforced")) {
                ...
                //    console.error("Full loop modified max_iob", getTddAdjustedMaxIob(profile, value, target_bg), "to effectively", round(getTddAdjustedMaxIob(profile, value, target_bg)*profile.profile_percentage/100,1), "due to profile percentage");
                //}
