...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (insulinReq > max_iob) {
            rT.reason += "maxIOB " + max_iob + ", ";
            console.error("InsReq " + round(insulinReq,2) + " capped at " + round(max_iob,2) + " to not exceed maxIOB");
            insulinReq = max_iob;
        } else { console.error("SMB not limited by maxIOB (insulinReq: " + insulinReq + " U)");}

        if (insulinForManualBolus > max_iob) {
            console.error("Ev. Bolus limited by maxIOB to " + round(max_iob,2) + " (insulinForManualBolus: " + insulinForManualBolus + " U)");
