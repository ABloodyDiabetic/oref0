...
function loop_smb(microBolusAllowed, profile, iob_data, aimismb, iobTH_reduction_ratio) {
    ...
    //     smbreason = ", autoISF-SMB disabled:, IOB: " + round(iob_data.iob,2) + ", > threshold: " + profile.iob_threshold + ", maxIOB: " + getTddAdjustedMaxIob(profile, glucose_status, target_bg);
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || (profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always && !profile.temptargetSet))  {
        ...
        if ( !evenTarget ){
            ...
            console.error("SMB disabled by Full Loop logic: IOB " + iob_data.iob + " is more than " + iobTHeffective + "% of maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg));
