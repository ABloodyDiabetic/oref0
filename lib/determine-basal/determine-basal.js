...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    // #### mod 14k: Add a switch between Autosens ISF and profile.sens
    ...
    function twoOrThreeStepfinalISF(profile, autosens_data) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens && autosens_data && autosens_data.ratio) {
        ...
        isfreason += ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + twoOrThreeStepfinalISF(profile, autosens_data);
        return autoISFsens;
    ...
    function twoOrThreeStepfinalISF2(profile, autosens_data) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens && autosens_data && autosens_data.ratio) {
            return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile, autosens_data),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        }
        return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(autoISFsens, profile);
    } // This function uses the "Use Autosens ISF to Calculate the autoISF Sens" toggle value to determine if the Autosens ISF should be included in the 'final ISF' progression. Two step or three step relative to the toggle.      
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + twoOrThreeStepfinalISF2(profile, autosens_data);
        return autoISFsens;
    }
