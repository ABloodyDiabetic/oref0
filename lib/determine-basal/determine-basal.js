...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > getTddAdjustedMaxIob(profile, glucose_status, target_bg)) {
        ...
        if (insulinReq > getTddAdjustedMaxIob(profile, glucose_status, target_bg)) {
            rT.reason += "maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg) + ", ";
            console.error("InsReq " + round(insulinReq,2) + " capped at " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg),2) + " to not exceed maxIOB");
            insulinReq = getTddAdjustedMaxIob(profile, glucose_status, target_bg);
        } else { console.error("SMB not limited by maxIOB (insulinReq: " + insulinReq + " U)");}

        if (insulinForManualBolus > getTddAdjustedMaxIob(profile, glucose_status, target_bg)) {
            console.error("Ev. Bolus limited by maxIOB to " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg),2) + " (insulinForManualBolus: " + insulinForManualBolus + " U)");
