...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    let final_ISF;
        ...
        const baseISF = withinISFlimits(liftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget);

        if (profile.low_carb_ez_fcl_profile) {
            final_ISF = ((baseISF - 1) * profile.low_carb_ez_fcl_profile_multiplier) + 1;
        } else if (profile.moderate_carb_ez_fcl_profile) {
            final_ISF = ((baseISF - 1) * profile.moderate_carb_ez_fcl_profile_multiplier) + 1;
        } else if (profile.high_carb_ez_fcl_profile) {
            final_ISF = ((baseISF - 1) * profile.high_carb_ez_fcl_profile_multiplier) + 1;
        } else {
