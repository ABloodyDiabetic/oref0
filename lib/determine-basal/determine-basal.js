...
function withinISFlimits(liftISF, minISFReduction, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget) {
    ...
    if ( high_temptarget_raises_sensitivity && profile.temptargetSet && target_bg > normalTarget ) {
         ...
         console.error("autoISF adjusts sens " + sensitivityRatio + ", instead of (profile.sens / autosens_data.ratio) "  + (profile.sens / autosens_data.ratio));
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (bg_ISF<1) {
        ...
        autoISFsens = Math.min(720, round((profile.sens / autosens_data.ratio) / final_ISF, 1));
        //isfreason +=  ", bg-ISF Ratio: " + round(final_ISF,2);
        isfreason += ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + convert_bg(profile.sens, profile) + "\u2192" + convert_bg((profile.sens / autosens_data.ratio),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        return autoISFsens;
    ...
    if ( sens_modified ) {
        ...
        final_ISF = withinISFlimits(liftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget);
        autoISFsens = round((profile.sens / autosens_data.ratio) / final_ISF, 1);
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + convert_bg((profile.sens / autosens_data.ratio),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        return autoISFsens;
    }
...
// Adjust ISF based on sensitivityRatio
    var profile_sens = round((profile.sens / autosens_data.ratio),1);
    var sens = (profile.sens / autosens_data.ratio);
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        sens = (profile.sens / autosens_data.ratio) / sensitivityRatio;
        sens = round(sens, 1);
...
// This has to happen after we obtain iob_data
    ...
    } else { // if IOB is negative, be more conservative and use the lower of sens, (profile.sens / autosens_data.ratio)
        naive_eventualBG = round( bg - (iob_data.iob * Math.min(sens, (profile.sens / autosens_data.ratio)) ) );
    }
...
// Initialize and calculate variables used for predicting BGs
    ...
    console.error("(profile.sens / autosens_data.ratio):" + convert_bg((profile.sens / autosens_data.ratio),profile) +", sens:" + convert_bg(sens,profile) + ", CSF:" + round(csf,1));
...
// Print summary variables based on predBGs etc.
    ...
    rT.auto_ISFratio = round((profile.sens / autosens_data.ratio) / sens, 2);
