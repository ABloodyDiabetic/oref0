...
// Print summary variables based on predBGs etc.
    ...
    if (profile.adjust_basal_inversely_to_autosens_isf && autosens_data && autosens_data.newisf){
        AutosensAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile),2) + " (tddISF: " + convert_bg(1700 / profile.calculate_isf_from_tdd_numerator_divisor / tdd,profile) + "), ";
    }
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (rate > maxSafeBasal * (profile.sens / getSensValue(profile))) {
            rT.reason += "adj. req. rate: " + round(rate,2) + " to maxSafeBasal: " + round(maxSafeBasal * (profile.sens / getSensValue(profile)),2) +", ";
            rate = round_basal(maxSafeBasal * (profile.sens / getSensValue(profile)), profile);
        }
