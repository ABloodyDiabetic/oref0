...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables, middleWare, value) {
   ...
   // -------------------- END OF TDD ----------------------------------------------------
    ...
    function getTddAdjustedMaxIob(profile, glucose_status, target_bg, value) {
        const currentBG = glucose_status.glucose;
      
        if (profile.enable_max_iob_deadbands && 
        Math.abs(currentBG - target_bg) <= (profile.tight_deadband_range / 100 * target_bg)) {
        return profile.max_iob_tight_deadband / 100 * ((tdd * 1) + (tdd_ytd * 0));
        } else if (profile.enable_max_iob_deadbands && 
               Math.abs(currentBG - target_bg) > (profile.tight_deadband_range /100 * target_bg) && 
               Math.abs(currentBG - target_bg) <= (profile.loose_deadband_range /100 * target_bg)) {
        return profile.max_iob_loose_deadband / 100 * ((tdd * 1) + (tdd_ytd * 0));
        } else {
        return profile.max_iob / 100 * ((tdd * 1) + (tdd_ytd * 0));
    }
} // This function modifies maxIOB to equal a percentage of the TDD to allow dynamic adjustment relative to TDD instead of being a fixed number. Everywhere you see getTddAdjustedMaxIob(profile, glucose_status, target_bg, value) is a location where 'profile.max_iob' was replaced.

    function getTddAdjustedMaxIobValue(profile, glucose_status, target_bg, value) {
        const currentBG = glucose_status.glucose;
...
function loop_smb(microBolusAllowed, profile, iob_data, aimismb, iobTH_reduction_ratio, value) {
    ...
    //     smbreason = ", autoISF-SMB disabled:, IOB: " + round(iob_data.iob,2) + ", > threshold: " + profile.iob_threshold + ", maxIOB: " + getTddAdjustedMaxIob(profile, glucose_status, target_bg, value);
    ...
    if ((profile.use_autoisf && profile.temptargetSet && profile.enableSMB_EvenOn_OddOff) || (profile.use_autoisf && profile.min_bg==profile.max_bg && profile.enableSMB_EvenOn_OddOff_always && !profile.temptargetSet))  {
        ...
        if ( !evenTarget ){
            ...
            return "blocked";
        } else if ( getTddAdjustedMaxIob(profile, glucose_status, target_bg, value)==0 ) {
            console.error("SMB disabled because of maxIOB=0")
            return "blocked";
        } else if (iobTHeffective/100 < iob_data.iob/(getTddAdjustedMaxIob(profile, glucose_status, target_bg, value)*iobTH_reduction_ratio)) {
            console.error("iobTH: " + round(iobTHeffective,1) + "%, IOB% of maxIOB at " + round(iob_data.iob/(getTddAdjustedMaxIob(profile, glucose_status, target_bg, value)*iobTH_reduction_ratio)*100,1) + "%")
            if (iobTH_reduction_ratio != 1) {
                console.error("Full Loop modified maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg, value) + " to effectively " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg, value)*iobTH_reduction_ratio,2) + " due to profile % and/or exercise mode");
                maxIOBreason = ", effective maxIOB " + round(getTddAdjustedMaxIob(profile, glucose_status, target_bg, value)*iobTH_reduction_ratio,2);
            } else {
                maxIOBreason = ", maxIOB "+ getTddAdjustedMaxIob(profile, glucose_status, target_bg, value);
            }
            smbreason = ", autoISF-SMB disabled:, iobTH exceeded";
            console.error("SMB disabled by Full Loop logic: IOB " + iob_data.iob + " is more than " + iobTHeffective + "% of maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg, value));
...
// Cancel high temps (and replace with neutral) or shorten long zero temps for various error conditions
    ...
    var max_iob = getTddAdjustedMaxIob(profile, glucose_status, target_bg, value); // maximum amount of non-bolus IOB OpenAPS will ever deliver
...
// Print summary variables based on predBGs etc.
    ...
    let maxIOBreason2 = "";
    if (profile.enable_max_iob_deadbands){
        maxIOBreason2 = "maxIOB: " + getTddAdjustedMaxIob(profile, glucose_status, target_bg) + " ( " + getTddAdjustedMaxIobValue(profile, glucose_status, target_bg) + "% of TDD), ";
    }
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            var iobTHvirtual = iob_threshold_percent/100*iobTHtolerance/100 * getTddAdjustedMaxIob(profile, glucose_status, target_bg, value) * iobTH_reduction_ratio;
            if (microBolus > iobTHvirtual - iob_data.iob && (loop_wanted_smb=="fullLoop" || loop_wanted_smb=="enforced")) {
                ...
                //    console.error("Full loop modified max_iob", getTddAdjustedMaxIob(profile, glucose_status, target_bg, value), "to effectively", round(getTddAdjustedMaxIob(profile, glucose_status, target_bg, value)*profile.profile_percentage/100,1), "due to profile percentage");
                //}
