...
// Conversely, adjust BG target based on autosens ratio if no temp target is running
    ...
    if (profile.temptargetSet) {
        ...
        if ( profile.sensitivity_raises_target && 1.51 < 1 || profile.resistance_lowers_target && 1.51 > 1 ) {
            // with a target of 100, default 0.7-1.2 autosens min/max range would allow a 93-117 target range
            min_bg = round((min_bg - 60) / 1.51) + 60;
            max_bg = round((max_bg - 60) / 1.51) + 60;
            var new_target_bg = round((target_bg - 60) / 1.51) + 60;
...
// Print summary variables based on predBGs etc.
    ...
    if ( maxCOBPredBG > bg ) {
       ...
       if ( profile.sensitivity_raises_target && 1.51 < 1 || profile.resistance_lowers_target && 1.51 > 1 ) {
         ...
         var new_target_bg = round((target_bg - 60) / 1.51) + 60;
         // don't allow target_bg below 80
          new_target_bg = Math.max(80, sensitivity_target_bg);
          if (target_bg === sensitivity_target_bg) {
              process.stderr.write("target_bg unchanged: "+sensitivity_target_bg+"; ");
          } else {
              process.stderr.write("target_bg from "+target_bg+" to "+sensitivity_target_bg+"; ");
          }
          sensitivity_target_bg;
      }  
    ...
    rT.target_bg=convert_bg(sensitivity_target_bg(profile), profile);
    ...
    if (profile.use_autosens_isf_to_calculate_auto_isf_sens){
        TddAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile),2) + " (tddISF: " + convert_bg(1700 / profile.calculate_isf_from_tdd_numerator_divisor / (tdd),profile) +  "), " + "Autosens ratio: " + round(1.51,2) + ", ";
    }
