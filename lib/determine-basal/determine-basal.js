...
var place_holder = function place_holder(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables, middleWare) {
   ...
   const tdd_ytd1 = oref2_variables.tddYtd;
   const average_7days1 = oref2_variables.tdd7d;
   const tempHBT1 = oref2_variables.hbt;
   const tempHBTset1 = oref2_variables.isEnabled;
   const avgDelta1 = glucose_status.avgdelta;


   function addTimeToDate1(objDate, _hours) {
       var ms1 = objDate.getTime();
       var add_ms1 = _hours * 36e5;
       var newDateObj1 = new Date(ms1 + add_ms1);
       return newDateObj1;
   }

   function subtractTimeFromDate1(date, hours_) {
       var ms_1 = date.getTime();
       var add_ms_1 = hours_ * 36e5;
       var new_date1 = new Date(ms_1 - add_ms_1);
       return new_date1;
   }

   function accountForIncrements1(insulin1) {
    ...
    var minimalDose1 = profile.bolus_increment;
    if (minimalDose1 != 0.025) {
        minimalDose1 = 0.05;
    }
    var incrementsRaw1 = insulin1 / minimalDose1;
    if (incrementsRaw1 >= 1) {
        var incrementsRounded1 = Math.floor(incrementsRaw1);
        return round(incrementsRounded1 * minimalDose1, 5);
    } else { return 0; }
   }

   function makeBaseString1(base_timeStamp) {
       function addZero1(i) {
           if (i < 10) { i = "0" + i }
           return i;
       }
       let hour1 = addZero(base_timeStamp.getHours());
       let minutes1 = addZero(base_timeStamp.getMinutes());
       let seconds1 = "00";
       let string1 = hour + ":" + minutes + ":" + seconds;
       return string1;
   }

   function timeDifferenceOfString1(string1, string2) {
       //Base time strings are in "00:00:00" format
       var time11 = new Date("1/1/1999 " + string1);
       var time21 = new Date("1/1/1999 " + string2);
       var ms11 = time11.getTime();
       var ms21 = time21.getTime();
       var difference1 = (ms11 - ms21) / 36e5;
       return difference1;
   }

   function calcScheduledBasalInsulin1(lastRealTempTime, addedLastTempTime) {
       var totalInsulin1 = 0;
       var old1 = addedLastTempTime;
       var totalDuration1 = (lastRealTempTime - addedLastTempTime) / 36e5;
       var basDuration1 = 0;
       var totalDurationCheck1 = totalDuration;
       var durationCurrentSchedule1 = 0;

       do {

           if (totalDuration1 > 0) {

               var baseTime_1 = makeBaseString(old);

               //Default basalrate in case none is found...
               var basalScheduledRate_1 = basalprofile[0].rate;
               for (let m1 = 0; m1 < basalprofile.length; m++) {

                   var timeToTest1 = basalprofile[m].start;

                   if (baseTime_1 == timeToTest1) {

                       if (m1 + 1 < basalprofile.length) {
                           let end1 = basalprofile[m1+1].start;
                           let start1 = basalprofile[m1].start;

                           durationCurrentSchedule1 = timeDifferenceOfString(end, start);

                           if (totalDuration1 >= durationCurrentSchedule1) {
                               basDuration1 = durationCurrentSchedule1;
                           } else if (totalDuration1 < durationCurrentSchedule1) {
                               basDuration1 = totalDuration1;
                           }

                       }
                       else if (m1 + 1 == basalprofile.length) {
                           let end1 = basalprofile[0].start;
                           let start1 = basalprofile[m1].start;
                           // First schedule is 00:00:00. Changed places of start and end here.
                           durationCurrentSchedule1 = 24 - (timeDifferenceOfString(start, end));
                       ...
                       basalScheduledRate_1 = basalprofile[m].rate;
                       totalInsulin1 += accountForIncrements(basalScheduledRate_1 * basDuration1);
                       totalDuration1 -= basDuration1;
                       //console.log("Dynamic ratios log: scheduled insulin added: " + accountForIncrements(basalScheduledRate_1 * basDuration1) + " U. Bas duration: " + basDuration1.toPrecision(3) + " h. Base Rate: " + basalScheduledRate_1 + " U/h" + ". Time :" + baseTime_1);
                       // Move clock to new date
                       old1 = addTimeToDate(old1, basDuration1);
                   }

                   else if (baseTime_1 > timeToTest1) {

                       if (m1 + 1 < basalprofile.length) {
                           var timeToTest21 = basalprofile[m1+1].start

                           if (baseTime_1 < timeToTest21) {

                              //  durationCurrentSchedule1 = timeDifferenceOfString(end, start);
                              durationCurrentSchedule1 = timeDifferenceOfString(timeToTest21, baseTime_1);
                               ...
                               basalScheduledRate_1 = basalprofile[m1].rate;
                               totalInsulin1 += accountForIncrements(basalScheduledRate_1 * basDuration1);
                               totalDuration1 -= basDuration1;
                               //console.log("Dynamic ratios log: scheduled insulin added: " + accountForIncrements(basalScheduledRate_1 * basDuration1) + " U. Bas duration: " + basDuration1.toPrecision(3) + " h. Base Rate: " + basalScheduledRate_1 + " U/h" + ". Time :" + baseTime_1);
                               // Move clock to new date
                       ...
                       else if (m1 == basalprofile.length - 1) {
                           // let start = basalprofile[m1].start;
                           let start1 = baseTime_1;
                           // First schedule is 00:00:00. Changed places of start and end here.
                           durationCurrentSchedule1 = timeDifferenceOfString("23:59:59", start);
       ...
       } while (totalDuration1 > 0 && totalDuration1 < totalDurationCheck1);

       // amount of insulin according to pump basal rate schedules
       return totalInsulin1;
   }
   ...
   if (!pumphistory.length) {
       ...
       let phLastEntry1 = pumphistory.length - 1;
       var endDate1 = new Date(pumphistory[phLastEntry1].timestamp);
       var startDate1 = new Date(pumphistory[0].timestamp);
       ...
       if (pumpData < 23.9 && pumpData > 21) {
           var missingHours1 = 24 - pumpData;
           // Makes new end date for a total time duration of exakt 24 hour.
           var endDate_1 = subtractTimeFromDate(endDate1, missingHours1);
           // endDate1 - endDate_1 = missingHours
           scheduledBasalInsulin1 = calcScheduledBasalInsulin(endDate1, endDate_1);
   ...
   var PHlastBolus1 = 0;
   var PHlastBolusAge1 = 0;
   round(( new Date(systemTime).getTime() - meal_data.lastBolusNormalTime ) / 60000,1)
   //Bolus:
   for (let i1 = 0; i1 < pumphistory.length; i1++) {
       if (pumphistory[i1]._type == "Bolus") {
           bolusInsulin1 += pumphistory[i1].amount;
           if (PHlastBolus1 == 0 && pumphistory[i1].amount >= profile.iTime_Start_Bolus) {
            PHlastBolus1 = round_basal(pumphistory[i1].amount,profile);
            var PHBolusTime1  = new Date(pumphistory[i1].timestamp);
            var currentDate1 =  new Date();
            PHlastBolusAge1 = round((currentDate1 - PHBolusTime1) / 36e5 * 60);
           }
       }
   }

   // Temp basals:
   for (let j1 = 1; j1 < pumphistory.length; j1++) {
       if (pumphistory[j1]._type == "TempBasal" && pumphistory[j1].rate > 0) {
           current1 = j1;
           quota = pumphistory[j1].rate;
           var duration1 = pumphistory[j1-1]['duration (min)'] / 60;
           var origDur1 = duration1;
           var pastTime1 = new Date(pumphistory[j1-1].timestamp);
           var morePresentTime1 = pastTime1;
           // If temp basal hasn't yet ended, use now as end date for calculation
           do {
               j1--;
               if (j1 == 0) {
                   morePresentTime1 =  new Date();
                   break;
               } else if (pumphistory[j1]._type == "TempBasal" || pumphistory[j1]._type == "PumpSuspend") {
                       morePresentTime1 = new Date(pumphistory[j1].timestamp);
                       break;
                 }
           }
           while (j1 > 0);

           var diff1 = (morePresentTime1 - pastTime1) / 36e5;
           if (diff1 < origDur1) {
               duration1 = diff1;
           }

           insulin1 = quota * duration;
           tempInsulin1 += accountForIncrements(insulin1);
           j1 = current;
       }
   }
   ...
   //
   for (let k1 = 0; k1 < pumphistory.length; k1++) {
       // Check for 0 temp basals with 0 min duration.
       insulin1 = 0;
       if (pumphistory[k1]['duration (min)'] == 0 || pumphistory[k1]._type == "PumpResume") {
           let time11 = new Date(pumphistory[k1].timestamp);
           let time21 = time11;
           let l = k1;
           do {
               if (l > 0) {
                   ...
                   if (pumphistory[l]._type == "TempBasal") {
                       time21 = new Date(pumphistory[l].timestamp);
                       break;
                   }
               }
           } while (l > 0);
           // duration of current1 scheduled basal in h
           let basDuration1 = (time21 - time11) / 36e5;

           if (basDuration1 > 0) {
               scheduledBasalInsulin1 += calcScheduledBasalInsulin(time21, time11);
           }
       }
   }

   // Check for temp basals that completes
   for (let n1 = pumphistory.length -1; n1 > 0; n1--) {
       if (pumphistory[n1]._type == "TempBasalDuration") {
           // duration in hours
           let oldBasalDuration1 = pumphistory[n1]['duration (min)'] / 60;
           // time of old temp basal
           let oldTime1 = new Date(pumphistory[n1].timestamp);
           var newTime1 = oldTime1;
           let o1 = n1;
           do {
               --o1;
               if (o1 >= 0) {
                   if (pumphistory[o1]._type == "TempBasal" || pumphistory[o1]._type == "PumpSuspend") {
                       // time of next (new) temp basal or a pump suspension
                       newTime1 = new Date(pumphistory[o1].timestamp);
                       break;
                   }
               }
           } while (o1 > 0);
           ...
           if (n1 == 0 && pumphistory[0]._type == "TempBasalDuration") {
               newTime1 = new Date();
               oldBasalDuration1 = pumphistory[n1]['duration (min)'] / 60;
           }

           let tempBasalTimeDifference1 = (newTime1 - oldTime1) / 36e5;
           let timeOfbasal1 = tempBasalTimeDifference1 - oldBasalDuration1;
           // if duration of scheduled basal is more than 0
           if (timeOfbasal1 > 0) {
               // Timestamp after completed temp basal
               let timeOfScheduledBasal1 =  addTimeToDate(oldTime1, oldBasalDuration1);
               scheduledBasalInsulin1 += calcScheduledBasalInsulin(newTime1, timeOfScheduledBasal1);
           }
       }
   }

   tdd1 = bolusInsulin1 + tempInsulin1 + scheduledBasalInsulin1;

   var insulin_1 = {
       TDD1: round(tdd1, 5),
       bolus1: round(bolusInsulin1, 5),
       temp_basal1: round(tempInsulin1, 5),
       scheduled_basal1: round(scheduledBasalInsulin1, 5)
   }
   var tdd_before1 = tdd1;

   if (pumpData > 21) {
       logBolus1 = ". Bolus insulin: " + bolusInsulin.toPrecision(5) + " U";
       logTempBasal1 = ". Temporary basal insulin: " + tempInsulin.toPrecision(5) + " U";
       logBasal1 = ". Insulin with scheduled basal rate: " + scheduledBasalInsulin.toPrecision(5) + " U";
       logtdd1 = " TDD past 24h is: " + tdd.toPrecision(5) + " U";
       logOutPut1 = dataLog + logtdd + logBolus + logTempBasal + logBasal;
       tddReason1 = ", TDD, 24h: " + round(tdd,1) + ", ytd: " + round(tdd_ytd,1)+ ", 7d\u00D8: " + round(average_7days,1);
       console.error(logOutPut);
   } else { tddReason1 = ", TDD: Not enough pumpData (< 21h)"; }

   // -------------------- END OF TDD ----------------------------------------------------

    function getSensValue(profile) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens) {
            return 1700 / round((tdd + tdd_ytd / 2),1);
        }
        return profile.sens;
    ...
    function getBasalValue(profile) {
        const originalBasal = profile.current_basal;
        const originalISF = profile.sens;

        if (profile.adjust_basal_inversely_to_autosens_isf) {
            const isfRatio = originalISF / (1700 / round((tdd + tdd_ytd / 2),1));
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    function twoOrThreeStepfinalISFReason(profile, autosens_data) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens && autosens_data && autosens_data.ratio) {
        ...
        isfreason += ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + twoOrThreeStepfinalISFReason(profile, autosens_data);
        return autoISFsens;
    ...
    function twoOrThreeStepfinalISFReason2(profile, autosens_data) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens && autosens_data && autosens_data.ratio) {
            return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        }
        return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(autoISFsens, profile);
    } // This function uses the "Use Autosens ISF to Calculate the autoISF Sens" toggle value to determine if the Autosens ISF should be included in the 'final ISF' progression. Two step or three step relative to the toggle.      
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + twoOrThreeStepfinalISFReason2(profile, autosens_data);
        return autoISFsens;
    }
...
// Print summary variables based on predBGs etc.
    ...
    let AutosensAdjustBasalreason = "";
    if (profile.adjust_basal_inversely_to_autosens_isf && autosens_data && autosens_data.newisf){
        AutosensAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile),2) + " (asISF: " + round(autosens_data.newisf,2) + "), " + "Autosens ratio: " + round(autosens_data.ratio,2) + ", ";
    }
  
    rT.reason =  MWreason + AutosensAdjustBasalreason + B30reason + SMBdelreason + autosensReason + TTreason + isfreason + ", Standard, COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI+ ", ISF: " + rT.ISF + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
