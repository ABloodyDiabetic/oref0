...
function convert_bg(value, profile)
{
    ...
    function getSensValue(profile, autosens_data) {
        if (profile.adjust_basal_inversely_to_autosens_isf && autosens_data && autosens_data.ratio) {
            return profile.sens / autosens_data.ratio;
        }
        return profile.sens;
    } // This function is the toggle to enable or disable "Use Autosens ISF to Calculate the autoISF Sens". 'getSensValue(profile, autosens_data)' has replaced 'profile.sens' in several places.

    function getBasalValue(profile, autosens_data) {
        ...
        if (profile.adjust_basal_inversely_to_autosens_isf && autosens_data && autosens_data.newisf) {
            const isfRatio = originalISF / autosens_data.newisf;
...
} // This function is the toggle to enable or disable "Adjust Basal Inversely to Autosens ISF". 'getBasalValue(profile, autosens_data)' replaces 'profile.current_basal' in several places.

    function adjustMaxSafeBasalInverselyToAutosensIsfDivisor(profile, autosens_data) {
        if (profile.adjust_basal_inversely_to_autosens_isf && autosens_data && autosens_data.newisf) {
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (bg_ISF<1) {
        ...
        autoISFsens = Math.min(720, round(getSensValue(profile, autosens_data) / final_ISF, 1));
    function twoOrThreeStepfinalISFReason(profile, autosens_data) {
        if (profile.use_autosens_isf_to_calculate_auto_isf_sens && autosens_data && autosens_data.ratio) {
            return convert_bg(profile.sens, profile) + "\u2192" + convert_bg(getSensValue(profile, autosens_data),profile) + "\u2192" + convert_bg(autoISFsens, profile);
        }
    ...
    if ( sens_modified ) {
        ...
        final_ISF = withinISFlimits(liftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget);
        autoISFsens = round(getSensValue(profile, autosens_data) / final_ISF, 1);
...
// Set variables required for evaluating error conditions
    ...
    if (typeof profile === 'undefined' || typeof getBasalValue(profile, autosens_data) === 'undefined') {
        rT.error ='Error: could not get current basal rate';
        return rT;
    }
    var profile_current_basal = round_basal(getBasalValue(profile, autosens_data), profile);
...
// Calculate sensitivityRatio based on temp targets, if applicable, or using the value calculated by autosens
    ...
    if (sensitivityRatio) {
        basal = getBasalValue(profile, autosens_data) * sensitivityRatio;
...
// Initialize and calculate variables used for predicting BGs
    ...
    console.error("profile.sens:" + convert_bg(getSensValue(profile, autosens_data),profile) +", sens:" + convert_bg(sens,profile) + ", CSF:" + round(csf,1));
...
// Print summary variables based on predBGs etc.
    ...
    rT.auto_ISFratio = round(getSensValue(profile, autosens_data) / sens, 2);
    ...
    if (profile.adjust_basal_inversely_to_autosens_isf && autosens_data && autosens_data.newisf){
        AutosensAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile, autosens_data),2) + " (asISF: " + round(autosens_data.newisf,2) + "), " + "Autosens ratio: " + round(autosens_data.ratio,2) + ", ";
    }
...
// Calculate carbsReq (carbs required to avoid a hypo)
    ...
    var zeroTempEffect = getBasalValue(profile, autosens_data)*sens*zeroTempDuration/60;
...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    var durationReq = 0;
    if (bg < threshold && iob_data.iob < -getBasalValue(profile, autosens_data)*20/60 && minDelta > 0 && minDelta > expectedDelta) {
        rT.reason += "IOB "+iob_data.iob+" < " + round(-getBasalValue(profile, autosens_data)*20/60,2);
    ...
    // predictive low glucose suspend mode: BG is / is projected to be < threshold
        ...
        durationReq = round(60*worstCaseInsulinReq / getBasalValue(profile, autosens_data));
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if (typeof profile.maxSMBBasalMinutes === 'undefined' ) {
                maxBolus = round(smb_max_range * getBasalValue(profile, autosens_data) * 30 / 60 ,1);
                console.error("profile.maxSMBBasalMinutes undefined: defaulting to 30m");
            //if (typeof profile.maxSMBBasalMinutes === 'undefined' ) {
            //    var maxBolus = round( getBasalValue(profile, autosens_data) * 30 / 60 ,1);
            //    console.error("profile.maxSMBBasalMinutes undefined: defaulting to 30m");
            // if IOB covers more than COB, limit maxBolus to 30m of basal
                ...
                if (profile.maxUAMSMBBasalMinutes) {
                    console.error("profile.maxUAMSMBBasalMinutes:",profile.maxUAMSMBBasalMinutes,"getBasalValue(profile, autosens_data):",getBasalValue(profile, autosens_data));
                    maxBolus = round( smb_max_range * getBasalValue(profile, autosens_data) * profile.maxUAMSMBBasalMinutes / 60 ,1);
                } else {
                    console.error("profile.maxUAMSMBBasalMinutes undefined: defaulting to 30m");
                    maxBolus = round( getBasalValue(profile, autosens_data) * 30 / 60 ,1);
                }
            } else {
                console.error("profile.maxSMBBasalMinutes:",profile.maxSMBBasalMinutes,"getBasalValue(profile, autosens_data):",getBasalValue(profile, autosens_data));
                maxBolus = round( smb_max_range * getBasalValue(profile, autosens_data) * profile.maxSMBBasalMinutes / 60 ,1);
            }
        ...
        var maxSafeBasal = tempBasalFunctions.getMaxSafeBasal(profile) / adjustMaxSafeBasalInverselyToAutosensIsfDivisor(profile, autosens_data);
