...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, oref2_variables, middleWare) {
   ...
   // -------------------- END OF TDD ----------------------------------------------------
    ...
    function getSensValue(profile, value) {
        const tddAdjustedISF = 1700 / profile.calculate_isf_from_tdd_numerator_divisor / tdd;
...
function getTddAdjustedMaxIob(profile, glucose_status, target_bg) {
    ...
    if (profile.enable_max_iob_deadbands) {
        ...
        if (currentBG >= 0 && currentBG < tightBandUpperLimit && !profile.moderate_carb_ez_fcl_profile && !profile.high_carb_ez_fcl_profile) {
            ...
            return profile.max_iob_tight_deadband / 100 * tdd;
            ...
            return profile.max_iob_loose_deadband / 100 * tdd;
    ...
    return profile.max_iob / 100 * tdd;
...
function resetEzFclProfileToDefaultWhenNoTempTargetsAreActive(profile) {
    // Check if the temporary target is not set
    if (!profile.temptargetSet) {
        // Check if the low carb profile is already set to the default and others are disabled
        const isAlreadyDefault = profile.low_carb_ez_fcl_profile === true &&
                                 profile.moderate_carb_ez_fcl_profile === false &&
                                 profile.high_carb_ez_fcl_profile === false;

        // If the profiles are not in the default state, reset them
        if (!isAlreadyDefault) {
            profile.low_carb_ez_fcl_profile = true;
            profile.moderate_carb_ez_fcl_profile = false;
            profile.high_carb_ez_fcl_profile = false;
            console.error('EZ FCL Profile has been reset to default settings.');
        } else {
            console.error('EZ FCL Profile is already at default settings.');
        }
        return true; // Indicate that the profile was checked and potentially modified
    }
    return false; // Indicate that no changes were made because a temp target is active
} // This function resets the profile configuration to the default profile after a temp target has ended ensuring that the profile selected in temp targets does not run indefinitely.
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        sens_modified = true;
        duraisfreason = ", Duration: " + dura05 + ", Avg: " + convert_bg(avg05,profile) + ", --- ISF Ratio: " + round(dura_ISF,2);
...
// Print summary variables based on predBGs etc.
    ...
    if (profile.use_autosens_isf_to_calculate_auto_isf_sens){
        TddAdjustBasalreason = "Basal: " + round(profile.current_basal,2) + "\u2192" + round(getBasalValue(profile),2) + " (tddISF: " + convert_bg(1700 / profile.calculate_isf_from_tdd_numerator_divisor / (tdd),profile) + "), ";
    }
