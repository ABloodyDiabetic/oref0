...
{   // #### mod 7e: added switch for autoISF ON/OFF
  ...
  function getSensValue(profile, autosens_data, preferences) {
    if (!preferences) {
        console.error("Error: Preferences are not provided to getSensValue");
        return profile.sens; // Return default sensitivity if preferences are not provided
    }
    ...
    if (useAutosensIsfToCalculateAutoIsfSens && autosens_data && autosens_data.ratio) {
        return profile.sens / autosens_data.ratio; // Use adjusted sensitivity based on autosens ratio
    }

    return profile.sens; // Return default sensitivity if the condition is not met
    let sensValue = getSensValue(profile, autosens_data, preferences);
    console.log("Calculated sensitivity:", sensitivity);
} // end of mod V14k code block
    ...
    if (bg_ISF<1) {
        ...
        autoISFsens = Math.min(720, round(sensValue / final_ISF, 1));
        //isfreason +=  ", bg-ISF Ratio: " + round(final_ISF,2);
        isfreason += ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + convert_bg(profile.sens, profile) + "\u2192" + convert_bg(sensValue,profile) + "\u2192" + convert_bg(autoISFsens, profile);
        return autoISFsens;
    ...
    if ( sens_modified ) {
        ...
        final_ISF = withinISFlimits(liftISF, profile.autoISF_min, maxISFReduction, sensitivityRatio, origin_sens, profile, high_temptarget_raises_sensitivity, target_bg, normalTarget);
        autoISFsens = round(sensValue / final_ISF, 1);
        isfreason += ppisfreason + deltaisfreason + duraisfreason + ", final Ratio: " + round(final_ISF,2) + exerciseReason + withinlimitsreason + ", final ISF: " + convert_bg(profile.sens, profile) + "\u2192" + convert_bg(sensValue,profile) + "\u2192" + convert_bg(autoISFsens, profile);
        return autoISFsens;
    }
...
// Calculate sensitivityRatio based on temp targets, if applicable, or using the value calculated by autosens
    ...
    else if (typeof autosens_data !== 'undefined' && autosens_data && profile.enable_autosens && sensValue !== profile.sens) {
...
// Initialize and calculate variables used for predicting BGs
    ...
    console.error("profile.sens:" + convert_bg(sensValue,profile) +", sens:" + convert_bg(sens,profile) + ", CSF:" + round(csf,1));
...
// Print summary variables based on predBGs etc.
    ...
    rT.auto_ISFratio = round(sensValue / sens, 2);
