...
// Begin core dosing logic: check for situations requiring low or high temps, and return appropriate temp after first match
    ...
    if (iob_data.iob > getTddAdjustedMaxIob(profile, glucose_status, target_bg)) {
        rT.reason += "IOB " + round(iob_data.iob,2) + " > maxIOB " + convert_bg(getTddAdjustedMaxIob(profile, glucose_status, target_bg), profile);
        if (currenttemp.duration > 15 && (round_basal(basal, profile) === round_basal(currenttemp.rate, profile))) {
            rT.reason += ", temp " + currenttemp.rate + " ~ req " + basal + "U/hr. ";
            return rT;
        } else {
        ...
        if (insulinReq > getTddAdjustedMaxIob(profile, glucose_status, target_bg) - iob_data.iob) {
            rT.reason += "maxIOB " + getTddAdjustedMaxIob(profile, glucose_status, target_bg) + ", ";
