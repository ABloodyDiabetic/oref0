...
//console.error("skipping:",glucose_data[i].glucose,glucose_data[i-1].glucose);
    ...
    historicalRatios.push({timestamp: new Date(), ratio: ratio});
    
    if (historicalRatios.length > 72) { // For example, keep only last 36 entries
        historicalRatios.shift(); // Removes the oldest entry
    }
